...
# Description: Import re, use re.sub() to eliminate chars that are not considered. re.sub(r'[^0-9a-zA-Z]','',str) => eliminate chars except [0-9][a-z][A-Z] and 
#              replace with empty string "". Then check if this processed string is a palindrome
import re
def isPalindromeSentence(s):
    trimmed = re.sub(r'[^a-zA-Z0-9]','',s).lower()         # eliminate characters except [0-9] [a-z] and [A-Z], and convert to lower case 
    l, r = 0, len(trimmed)-1
    while l<r:                        # check if [trim] is palindrome
...
def findSecondMinimumValue(root):
    res = [float("inf")]
    findSecondMinimumValue_helper(root, res)
...
class KthLargest:
    def __init__(self, k, nums):
        self.k = k
        self.pool = heapq.heapify(nums)         # convert [nums] as min-heap (linear time)
    ...
    def add(self, val):
        ...
        # if no more empty space, and inserting [val] is larger than minimum value in [heap]
            ...
            heapq.heappushpop(val)              # "heappushpop(val)" firstly push(val), then pop and return min
